    ...
    crate::default_issue_callbacks::DefaultIssueCallbacksModule + crate::pause::PauseModule
{
    ...
    fn merge_tokens(&self) -> DctTokenPayment<Self::Api> {
    ...
    fn split_tokens(&self) -> MultiValueEncoded<DctTokenPayment<Self::Api>> {
        self.require_not_paused();
        ...
        for token in &payments {
            ...
            for inst in previously_merged_instance_attributes {
                let original_token = DctTokenPayment::new(
                    TokenIdentifier::from_dct_bytes(inst.original_token_id_raw.as_slice()),
    ...
    fn split_token_partial(
        &self,
        tokens_to_remove: MultiValueEncoded<DctTokenPayment<Self::Api>>,
    ) -> MultiValueEncoded<DctTokenPayment<Self::Api>> {
        self.require_not_paused();

        let merged_token = self.call_value().single_dct();
        self.merged_token()
